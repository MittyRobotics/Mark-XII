...
public class DriveAtom extends Atom {
	...
	public void init() {
		try {
			TKOHardware.getDriveTalon(0).changeControlMode(
					CANTalon.ControlMode.Position);
			TKOHardware.getDriveTalon(2).changeControlMode(
					CANTalon.ControlMode.Position);
			TKOHardware.getDriveTalon(0).setFeedbackDevice(
			...
			e.printStackTrace();
			System.out.println("HOLY FUCK THIS ENCODER DUN GOOFED UP!!!");
		}
	...
	public void execute() {
		...
		try {
			while (DriverStation.getInstance().isEnabled()) {
				...
				System.out.println("Ncoder Left: "
			...
			e1.printStackTrace();
			System.out
					.println("Error at another expected spot, I would assume....");
		}
