...
public class TKODrive implements Runnable {
	...
	public synchronized void currentModeTankDrive() {
		try {
			TKOHardware.changeTalonMode(TKOHardware.getLeftDrive(), CANTalon.ControlMode.Current, Definitions.DRIVE_P,
	...
	public synchronized void PIDCurrentCalibration() {
		...
		try {
			...
			// TODO first run does not actually go until one iteration of loop
			// (maybe fixed now)
			{
				...
				TKODataReporting.getInstance().startCollectingDriveData(p, i, d); // stops
																					// regular
																					// data
																					// collection
				...
				for (int j = 0; j < Definitions.NUM_DRIVE_TALONS; j += 2) {
					...
					if (p < 10)
						TKOLogger.getInstance().addData("MotorSetCommand", System.nanoTime(),
								"p: 0" + p + " i: 0" + i + " d: 0" + d, j);
					else
						TKOLogger.getInstance().addData("MotorSetCommand", System.nanoTime(),
								"p: " + p + " i: " + i + " d: " + d, j);
				...
				while ((System.currentTimeMillis() - start) < runningTime) {
					// record the point in time when feedback exceeds target, or
					// is within x% of target
				...
				TKODataReporting.getInstance().stopCollectingDriveData(); // starts
																			// regular
																			// data
																			// collection
				// p += 1.;
	...
	private void overTheLipPositioner() {
		try {
			...
			while ((TKOHardware.getCrateDistance() > Definitions.TRASHCAN_POSITIONING_MAX
					|| TKOHardware.getCrateDistance() < Definitions.TRASHCAN_POSITIONING_MIN) && t.get() < 10
					&& TKOHardware.getJoystick(1).getRawButton(3)) {
	...
	public synchronized void setLeftRightMotorOutputsCurrent(double leftMult, double rightMult) {
		try {
			if (TKOHardware.getLeftDrive().getControlMode() == CANTalon.ControlMode.Current) {
				TKOHardware.getLeftDrive()
						.set(Definitions.DRIVE_MULTIPLIER_LEFT * Definitions.MAX_CURRENT_LEFT * leftMult);
			} else
			...
			if (TKOHardware.getRightDrive().getControlMode() == CANTalon.ControlMode.Current)
				TKOHardware.getRightDrive()
						.set(Definitions.DRIVE_MULTIPLIER_RIGHT * Definitions.MAX_CURRENT_RIGHT * rightMult);
			else
				throw new TKORuntimeException("ERROR TRIED TO RUN TANK DRIVE ON NON-CURRENT TALON");
	...
	public void tankDrive() {
		try {
			// the change talon mode should only do anything if the mode is not
			// already that which it is trying to set
			TKOHardware.changeTalonMode(TKOHardware.getLeftDrive(), CANTalon.ControlMode.PercentVbus,
					Definitions.DRIVE_P, Definitions.DRIVE_I, Definitions.DRIVE_D); // TODO
																					// make
																					// sure
																					// this
																					// is
	...
	public void tankDriveJoystick() { // Note: this copies the drive system I
		...
		try {
			...
			TKOHardware.changeTalonMode(TKOHardware.getRightDrive(), CANTalon.ControlMode.PercentVbus,
					Definitions.DRIVE_P, Definitions.DRIVE_I, Definitions.DRIVE_D);
			TKOHardware.getLeftDrive().enableBrakeMode(true);
			// Here to make sure robot doesn't go insane
			TKOHardware.getRightDrive().enableBrakeMode(true);
